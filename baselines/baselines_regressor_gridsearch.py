...
def run_regressor(X_train_top, X_test_top, X_dev_top, y_train_df, y_test_df, y_dev_df, 
                   model_names:str='all', attr_setting:str='all', save=1):
    supported_models = {
    ...
    for model_name in model_names_list:
        ...
        if hasattr(best_model, 'predict_proba'):
            ...
            if y_pred_proba.shape[1] == 4:
                y_pred_score = np.argmax(y_pred_proba, axis=1) + 1

            elif y_pred_proba.shape[1] == 2:
                y_pred_binary = (y_pred_proba[:, 1] > 0.5).astype(int)
                y_pred_score = y_pred_binary + 2

            elif y_pred_proba.shape[1] == 1:
                y_pred_binary = (y_pred_proba[:, 0] > 0.5).astype(int)
                y_pred_score = y_pred_binary + 2
        ...
        metrics, cm = evaluate_predictions(y_test_df.values.ravel(), y_pred_score)
    ...
    if save:
        ...
        with open(f'{output_path.split(".")[0]}_{attr_setting}_grid.{output_path.split(".")[1]}', 'w') as f:
            json.dump(results, f, indent=4)

        save_multiple_matrices_to_csv(list(cms.values()), list(cms.keys()),
                                    f'{output_path.split(".")[0]}_cm_{attr_setting}_grid.csv')
